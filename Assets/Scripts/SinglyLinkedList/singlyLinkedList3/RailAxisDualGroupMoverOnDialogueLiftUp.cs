...
public class RailAxisDualGroupMoverOnDialogueLiftUp : MonoBehaviour
{
    ...
    public bool hideDropItemBeforeTrigger = true;
    ...
    void CacheInitial()
    {
        ...
        if (dropItem) _initialPos[dropItem] = dropItem.position;
        if (hideOnMoveStart) _initialActive[hideOnMoveStart] = hideOnMoveStart.activeSelf;
        if (dropItem) _initialActive[dropItem.gameObject] = dropItem.gameObject.activeSelf;
    ...
    void TakeSnapshot()
    {
        ...
        if (dropItem) _snapshotPos[dropItem] = dropItem.position;
        if (hideOnMoveStart) _snapshotActive[hideOnMoveStart] = hideOnMoveStart.activeSelf;
        if (dropItem) _snapshotActive[dropItem.gameObject] = dropItem.gameObject.activeSelf;
    ...
    void Awake()
    {
        ...
        if (hideDropItemBeforeTrigger && dropItem) dropItem.gameObject.SetActive(false);
    ...
    IEnumerator Sequence()
    {
        ...
        float t = 0f;
        if (hideDropItemBeforeTrigger && dropItem) dropItem.gameObject.SetActive(true);
    ...
    {
        if (!dropItem || dropDuration <= 0f || dropDistance <= 0f) yield break;
        float d = Mathf.Max(0f, dropDelay);
        float t = 0f;
        while (t < d) { t += useUnscaledTime ? Time.unscaledDeltaTime : Time.deltaTime; yield return null; }
        Vector3 start = dropItem.position;
        ...
        while (e < dur)
        {
            e += useUnscaledTime ? Time.unscaledDeltaTime : Time.deltaTime;
            float tt = Mathf.Clamp01(e / dur);
            dropItem.position = Vector3.Lerp(start, end, tt);
            yield return null;
        }
        dropItem.position = end;
    }
